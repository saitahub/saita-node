...
fn chunks(n_validators: usize, pov: &Vec<u8>) -> Vec<Vec<u8>> {
	saitama_erasure_coding::obtain_chunks(n_validators, pov).unwrap()
}

fn erasure_root(n_validators: usize, pov: &Vec<u8>) -> Hash {
	let chunks = chunks(n_validators, pov);
	saitama_erasure_coding::branches(&chunks).root()
}
	...
	for n_validators in N_VALIDATORS {
		...
		group.bench_with_input(
			...
			|b, &n| {
				b.iter(|| {
					let _pov: Vec<u8> =
						saitama_erasure_coding::reconstruct(n, last_chunks.clone()).unwrap();
				});
			},
		);
	}
