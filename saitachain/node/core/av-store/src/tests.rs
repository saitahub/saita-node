...
use saitama_node_primitives::{AvailableData, BlockData, PoV, Proof};
...
use saitama_node_subsystem_util::{database::Database, TimeoutExt};
...
fn test_harness<T: Future<Output = VirtualOverseer>>(
	...
	let _ = env_logger::builder()
		.is_test(true)
		.filter(Some("saitama_node_core_av_store"), log::LevelFilter::Trace)
...
fn test_store() -> Arc<dyn Database> {
	...
	let db =
		saitama_node_subsystem_util::database::kvdb_impl::DbAdapter::new(db, &[columns::META]);
	Arc::new(db)
}
