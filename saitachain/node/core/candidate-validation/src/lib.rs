...
use saitama_node_subsystem_util::executor_params_at_relay_parent;
...
pub struct CandidateValidationSubsystem {
	...
	pub metrics: Metrics,
	#[allow(missing_docs)]
	pub pvf_metrics: saitama_node_core_pvf::Metrics,
...
impl CandidateValidationSubsystem {
	...
	pub fn with_config(
		config: Option<Config>,
		metrics: Metrics,
		pvf_metrics: saitama_node_core_pvf::Metrics,
	) -> Self {
...
impl<Context> CandidateValidationSubsystem {
	fn start(self, ctx: Context) -> SpawnedSubsystem {
		if let Some(config) = self.config {
			...
			saitama_overseer::DummySubsystem.start(ctx)
		}
	}
}
...
async fn run<Context>(
	...
	let (validation_host, task) = saitama_node_core_pvf::start(
		saitama_node_core_pvf::Config::new(
...
impl ValidationBackend for ValidationHost {
	...
	async fn validate_candidate(
		...
		let priority = saitama_node_core_pvf::Priority::Normal;
