...
use saitama_node_primitives::DisputeStatus;
use saitama_node_subsystem_util::database::{DBTransaction, Database};
...
impl From<CandidateVotes> for saitama_node_primitives::CandidateVotes {
	fn from(db_votes: CandidateVotes) -> saitama_node_primitives::CandidateVotes {
		saitama_node_primitives::CandidateVotes {
...
impl From<saitama_node_primitives::CandidateVotes> for CandidateVotes {
	fn from(primitive_votes: saitama_node_primitives::CandidateVotes) -> CandidateVotes {
		CandidateVotes {
...
mod tests {
	...
	use saitama_node_primitives::DISPUTE_WINDOW;
	use saitama_primitives::{Hash, Id as ParaId};

	fn make_db() -> DbBackend {
		let db = kvdb_memorydb::create(1);
		let db = saitama_node_subsystem_util::database::kvdb_impl::DbAdapter::new(db, &[0]);
