...
use saitama_node_subsystem::overseer;
...
impl CandidateVoteState<CandidateVotes> {
	...
	pub fn new(votes: CandidateVotes, env: &CandidateEnvironment, now: Timestamp) -> Self {
		...
		let supermajority_threshold = saitama_primitives::supermajority_threshold(n_validators);
		...
		let (dispute_status, byzantine_threshold_against) = if is_disputed {
			...
			let byzantine_threshold = saitama_primitives::byzantine_threshold(n_validators);
