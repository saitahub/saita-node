...
use saitama_primitives::{Block, Hash, InherentData as ParachainsInherentData};
...
pub struct ParachainsInherentDataProvider<C: sp_blockchain::HeaderBackend<Block>> {
	pub client: Arc<C>,
	pub overseer: saitama_overseer::Handle,
	pub parent: Hash,
}

impl<C: sp_blockchain::HeaderBackend<Block>> ParachainsInherentDataProvider<C> {
	/// Create a new [`Self`].
	pub fn new(client: Arc<C>, overseer: saitama_overseer::Handle, parent: Hash) -> Self {
	...
	for ParachainsInherentDataProvider<C>
{
	async fn provide_inherent_data(
		...
		dst_inherent_data
			.put_data(saitama_primitives::PARACHAINS_INHERENT_IDENTIFIER, &inherent_data)
	}
