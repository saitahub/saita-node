...
//! SaitaChain Jaeger related primitives
//!
//! Provides primitives used by SaitaChain for interfacing with Jaeger.
//!
//! # Integration
//!
...
impl Jaeger {
		...
		match *instance {
			...
			_ => {
				let (traces_in, _traces_out) = mick_jaeger::init(mick_jaeger::Config {
					service_name: "saitachain-jaeger-test".to_owned(),
				});
	...
	pub fn launch<S: SpawnNamed>(self, spawner: S) -> result::Result<(), JaegerError> {
		...
		let (traces_in, mut traces_out) = mick_jaeger::init(mick_jaeger::Config {
			service_name: format!("saitachain-{}", cfg.node_name),
		});
