...
pub enum Error {
	#[error(transparent)]
	Subsystem(#[from] saitama_node_subsystem::SubsystemError),
	...
	#[error(transparent)]
	Runtime(#[from] saitama_node_subsystem::errors::RuntimeApiError),

	#[error(transparent)]
	Erasure(#[from] saitama_erasure_coding::Error),

	#[error(transparent)]
	Util(#[from] saitama_node_subsystem_util::Error),
}
