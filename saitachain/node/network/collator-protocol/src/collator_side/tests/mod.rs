...
use saitama_node_subsystem_util::{reputation::add_reputation, TimeoutExt};
...
use saitama_primitives_test_helpers::TestCandidateBuilder;
...
fn test_harness<T: Future<Output = TestHarness>>(
	...
	let _ = env_logger::builder()
		.is_test(true)
		.filter(Some("saitama_collator_protocol"), log::LevelFilter::Trace)
...
async fn connect_peer(
	...
	overseer_send(
		...
		CollatorProtocolMessage::NetworkBridgeUpdate(NetworkBridgeEvent::PeerConnected(
			peer,
			saitama_node_network_protocol::ObservedRole::Authority,
			version.into(),
