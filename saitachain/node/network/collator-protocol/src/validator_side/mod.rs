...
use saitama_node_primitives::{SignedFullStatement, Statement};
...
struct State {
	...
	/// [`saitama_node_network_protocol::View`] and can be dropped once the transition
...
{
	let validators = saitama_node_subsystem_util::request_validators(relay_parent, sender)
		.await
		.await
		.map_err(Error::CancelledActiveValidators)??;

	let (groups, rotation_info) =
		saitama_node_subsystem_util::request_validator_groups(relay_parent, sender)
			.await
			.await
			.map_err(Error::CancelledValidatorGroups)??;

	let cores = saitama_node_subsystem_util::request_availability_cores(relay_parent, sender)
		.await
		.await
		.map_err(Error::CancelledAvailabilityCores)??;

	let para_now = match saitama_node_subsystem_util::signing_key_and_index(&validators, keystore)
		.and_then(|(_, index)| saitama_node_subsystem_util::find_validator_group(&groups, index))
	{
		Some(group) => {
	...
	Sender: CollatorProtocolSenderTrait,
{
	...
	saitama_node_subsystem_util::request_persisted_validation_data(
		relay_parent,
		para_id,
