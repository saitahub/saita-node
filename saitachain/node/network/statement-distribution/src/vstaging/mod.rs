...
impl PerSessionState {
	fn new(session_info: SessionInfo, keystore: &KeystorePtr, backing_threshold: u32) -> Self {
		...
		let local_validator = saitama_node_subsystem_util::signing_key_and_index(
...
pub(crate) async fn handle_active_leaves_update<Context>(
	...
	for new_relay_parent in new_relay_parents.iter().cloned() {
		...
		let session_index = saitama_node_subsystem_util::request_session_index_for_child(
			new_relay_parent,
			ctx.sender(),
		)
		.await
		...
		let availability_cores = saitama_node_subsystem_util::request_availability_cores(
			new_relay_parent,
			ctx.sender(),
		)
		.await
		...
		let group_rotation_info =
			saitama_node_subsystem_util::request_validator_groups(new_relay_parent, ctx.sender())
				.await
				.await
				.map_err(JfyiError::RuntimeApiUnavailable)?
		...
		if !state.per_session.contains_key(&session_index) {
			let session_info = saitama_node_subsystem_util::request_session_info(
				new_relay_parent,
				session_index,
				ctx.sender(),
			)
