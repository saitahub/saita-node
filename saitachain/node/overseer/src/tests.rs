...
use saitama_node_network_protocol::{PeerId, UnifiedReputationChange};
...
use saitama_node_subsystem_types::{
	jaeger,
...
fn extract_metrics(registry: &prometheus::Registry) -> HashMap<&'static str, u64> {
	...
	let activated = extract("saitachain_parachain_activated_heads_total");
	let deactivated = extract("saitachain_parachain_deactivated_heads_total");
	let relayed = extract("saitachain_parachain_messages_relayed_total");
