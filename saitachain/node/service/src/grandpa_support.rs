...
//! SaitaChain-specific GRANDPA integration utilities.
...
mod tests {
	...
	use saitama_test_client::{
		ClientBlockImportExt, DefaultTestClientBuilderExt, InitSaitaChainBlockBuilder,
	...
	fn grandpa_pause_voting_rule_works() {
		...
		let mut push_blocks = {
			...
			move |hashes: &mut Vec<_>, n| {
				for _ in 0..n {
					let block = client.init_saitachain_block_builder().build().unwrap().block;
