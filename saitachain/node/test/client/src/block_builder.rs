...
use saitama_primitives::{Block, InherentData as ParachainsInherentData};
use saitachain_test_runtime::UncheckedExtrinsic;
use saitama_test_service::GetLastTimestamp;
...
/// An extension for the test client to initialize a SaitaChain specific block builder.
pub trait InitSaitaChainBlockBuilder {
	/// Init a SaitaChain specific block builder that works for the test runtime.
	///
	/// This will automatically create and push the inherents for you to make the block valid for
	/// the test runtime.
	fn init_saitachain_block_builder(
		&self,
	) -> sc_block_builder::BlockBuilder<Block, Client, FullBackend>;

	/// Init a SaitaChain specific block builder at a specific block that works for the test runtime.
	///
	/// Same as [`InitSaitaChainBlockBuilder::init_saitachain_block_builder`] besides that it takes a
	/// `Hash` to say which should be the parent block of the block that is being build.
	fn init_saitachain_block_builder_at(
		&self,
		hash: <Block as BlockT>::Hash,
	) -> sc_block_builder::BlockBuilder<Block, Client, FullBackend>;
}

impl InitSaitaChainBlockBuilder for Client {
	fn init_saitachain_block_builder(&self) -> BlockBuilder<Block, Client, FullBackend> {
		let chain_info = self.chain_info();
		self.init_saitachain_block_builder_at(chain_info.best_hash)
	}
	...
	fn init_saitachain_block_builder_at(
		...
		let minimum_period = BasicExternalities::new_empty()
			.execute_with(|| saitachain_test_runtime::MinimumPeriod::get());
		...
		let slot_duration = BasicExternalities::new_empty()
			.execute_with(|| saitachain_test_runtime::SlotDuration::get());
		...
		inherent_data
			.put_data(
				saitama_primitives::PARACHAINS_INHERENT_IDENTIFIER,
...
/// Saitama specific extensions for the [`BlockBuilder`].
pub trait BlockBuilderExt {
	/// Push a SaitaChain test runtime specific extrinsic to the block.
	///
