...
//! A SaitaChain test client.
//!
//! This test client is using the SaitaChain test runtime.

mod block_builder;

use saitama_primitives::Block;
use sp_runtime::BuildStorage;
use std::sync::Arc;

pub use block_builder::*;
pub use saitachain_test_runtime as runtime;
pub use saitama_test_service::{
...
/// Test client builder for SaitaChain.
...
impl substrate_test_client::GenesisInit for GenesisParameters {
	fn genesis_storage(&self) -> Storage {
		saitama_test_service::chain_spec::saitachain_local_testnet_genesis()
			.build_storage()
...
mod tests {
	...
	fn ensure_test_client_can_build_and_import_block() {
		...
		let block_builder = client.init_saitachain_block_builder();
	...
	fn ensure_test_client_can_push_extrinsic() {
		...
		block_builder.push_saitachain_extrinsic(transfer).expect("Pushes extrinsic");
