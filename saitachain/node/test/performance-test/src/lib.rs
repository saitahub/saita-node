...
//! A SaitaChain performance tests utilities.

use saitama_erasure_coding::{obtain_chunks, reconstruct};
use saitama_primitives::ExecutorParams;
use std::time::{Duration, Instant};

mod constants;

pub use constants::*;
pub use saitama_node_primitives::VALIDATION_CODE_BOMB_LIMIT;

/// Value used for reference benchmark of erasure-coding.
pub const ERASURE_CODING_N_VALIDATORS: usize = 1024;

pub use saitama_runtime::WASM_BINARY;
...
pub enum PerfCheckError {
	...
	#[error(transparent)]
	ErasureCoding(#[from] saitama_erasure_coding::Error),
...
pub fn measure_pvf_prepare(wasm_code: &[u8]) -> Result<Duration, PerfCheckError> {
	...
	let blob = saitama_node_core_pvf_prepare_worker::prevalidate(code.as_ref())
		.map_err(PerfCheckError::from)?;
	saitama_node_core_pvf_prepare_worker::prepare(blob, &ExecutorParams::default())
		.map_err(PerfCheckError::from)?;
