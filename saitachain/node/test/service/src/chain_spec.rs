...
use saitama_primitives::{AccountId, AssignmentId, ValidatorId, MAX_CODE_SIZE, MAX_POV_SIZE};
use saitama_service::chain_spec::{
	get_account_id_from_seed, get_from_seed, saitachain_chain_spec_properties, Extensions,
};
use saitachain_test_runtime::BABE_GENESIS_EPOCH_CONFIG;
use sc_chain_spec::{ChainSpec, ChainType};
use sp_authority_discovery::AuthorityId as AuthorityDiscoveryId;
use sp_core::sr25519;
use sp_runtime::Perbill;
use test_runtime_constants::currency::STCS;

const DEFAULT_PROTOCOL_ID: &str = "stc";

/// The `ChainSpec` parameterized for saitachain test runtime.
pub type SaitaChainChainSpec =
	sc_service::GenericChainSpec<saitachain_test_runtime::RuntimeGenesisConfig, Extensions>;

/// Local testnet config (multivalidator Alice + Bob)
pub fn saitachain_local_testnet_config() -> SaitaChainChainSpec {
	SaitaChainChainSpec::from_genesis(
		"Local Testnet",
		"local_testnet",
		ChainType::Local,
		|| saitachain_local_testnet_genesis(),
		vec![],
		None,
...
pub fn saitachain_local_testnet_genesis() -> saitachain_test_runtime::RuntimeGenesisConfig {
...
fn saitachain_testnet_genesis(
...
) -> saitachain_test_runtime::RuntimeGenesisConfig {
	use saitachain_test_runtime as runtime;

	let endowed_accounts: Vec<AccountId> = endowed_accounts.unwrap_or_else(testnet_accounts);

	const ENDOWMENT: u128 = 1_000_000 * STCS;
	const STASH: u128 = 100 * STCS;
	...
	runtime::RuntimeGenesisConfig {
		...
		configuration: runtime::ConfigurationConfig {
			config: saitama_runtime_parachains::configuration::HostConfiguration {
