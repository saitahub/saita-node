...
use saitama_node_primitives::{CollationGenerationConfig, CollatorFn};
use saitama_node_subsystem::messages::{CollationGenerationMessage, CollatorProtocolMessage};
use saitama_overseer::Handle;
use saitama_primitives::{Balance, CollatorPair, HeadData, Id as ParaId, ValidationCode};
use saitama_runtime_common::BlockHashCount;
use saitama_runtime_parachains::paras::{ParaGenesisArgs, ParaKind};
use saitama_service::{Error, FullClient, IsParachainNode, NewFull, PrometheusConfig};
use saitachain_test_runtime::{
...
pub use saitama_service::{FullBackend, GetLastTimestamp};
...
pub fn new_full(
	...
	saitama_service::new_full(
		config,
		saitama_service::NewFullParams {
			is_parachain_node,
			grandpa_pause: None,
			enable_beefy: true,
			jaeger_agent: None,
			...
			workers_path,
			workers_names: None,
			overseer_gen: saitama_service::RealOverseerGen,
...
/// Create a SaitaChain `Configuration`.
///
...
pub fn node_config(
	...
	let mut spec = saitachain_local_testnet_config();
	...
	Configuration {
		impl_name: "saitachain-test-node".to_string(),
...
pub fn run_validator_node(
	...
	let NewFull { task_manager, client, network, rpc_handlers, overseer_handle, .. } =
		new_full(config, IsParachainNode::No, worker_program_path)
			.expect("could not create SaitaChain test service");

	let overseer_handle = overseer_handle.expect("test node must have an overseer handle");
	let peer_id = network.local_peer_id();
	let addr = MultiaddrWithPeerId { multiaddr, peer_id };

	SaitaChainTestNode { task_manager, client, overseer_handle, addr, rpc_handlers }
}
...
/// [`SaitaChainTestNode::register_collator`].
...
/// A SaitaChain test node instance used for testing.
pub struct SaitaChainTestNode {
...
impl SaitaChainTestNode {
	/// Send a sudo call to this node.
	async fn send_sudo(
		&self,
		call: impl Into<saitachain_test_runtime::RuntimeCall>,
	...
	pub async fn send_extrinsic(
		&self,
		function: impl Into<saitachain_test_runtime::RuntimeCall>,
		caller: Sr25519Keyring,
...
pub fn construct_extrinsic(
	...
	UncheckedExtrinsic::new_signed(
		function.clone(),
		saitachain_test_runtime::Address::Id(caller.public().into()),
		saitama_primitives::Signature::Sr25519(signature.clone()),
		extra.clone(),
	)
}
...
pub fn construct_transfer_extrinsic(
	...
	let function =
		saitachain_test_runtime::RuntimeCall::Balances(pallet_balances::Call::transfer_allow_death {
