...
async fn collating_using_undying_collator() {
	...
	let alice_config = saitama_test_service::node_config(
		|| {},
	...
	// start alice
	let alice = saitama_test_service::run_validator_node(alice_config, Some(PUPPET_EXE.into()));

	let bob_config = saitama_test_service::node_config(
		|| {},
		tokio::runtime::Handle::current(),
		Bob,
	...
	// start bob
	let bob = saitama_test_service::run_validator_node(bob_config, Some(PUPPET_EXE.into()));
	...
	let mut charlie = saitama_test_service::run_collator_node(
		tokio::runtime::Handle::current(),
		Charlie,
		|| {},
