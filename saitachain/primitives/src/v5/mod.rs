...
pub use saitama_core_primitives::v2::{
...
// Export some saitachain-parachain primitives
pub use saitachain_parachain_primitives::primitives::{
...
pub mod well_known_keys {
	...
	///
	/// The storage entry stores a `Hash`. This is saitachain hash which is at the moment
	/// `blake2b-256`.
...
/// Custom validity errors used in SaitaChain while validating transactions.
#[repr(u8)]
...
/// Consensus engine id for saitachain v1 consensus engine.
pub const SAITACHAIN_ENGINE_ID: runtime_primitives::ConsensusEngineId = *b"POL1";

/// A consensus log item for saitachain validation. To be used with [`SAITACHAIN_ENGINE_ID`].
...
impl ConsensusLog {
	...
	pub fn from_digest_item(
		...
		match digest_item {
			runtime_primitives::DigestItem::Consensus(id, encoded) if id == &SAITACHAIN_ENGINE_ID =>
...
impl From<ConsensusLog> for runtime_primitives::DigestItem {
	fn from(c: ConsensusLog) -> runtime_primitives::DigestItem {
		Self::Consensus(SAITACHAIN_ENGINE_ID, c.encode())
	}
}
...
/// API. Reasoning and further outlook [here]
#[derive(Clone, Encode, Decode, RuntimeDebug, TypeInfo)]
#[cfg_attr(feature = "std", derive(PartialEq))]
