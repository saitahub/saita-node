...
//! Pallet to process purchase of STCs.
...
/// All information about an account regarding the purchase of STCs.
...
pub struct AccountStatus<Balance> {
	...
	/// The amount of free STCs they have purchased.
	free_balance: Balance,
	/// The amount of locked STCs they have purchased.
...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		/// The amount of purchased locked STCs that we will unlock for basic actions on the chain.
		#[pallet::constant]
		type UnlockedProportion: Get<Permill>;

		/// The maximum amount of locked STCs that we will unlock.
		#[pallet::constant]
	...
	// A map of all participants in the STC purchase process.
	#[pallet::storage]
	pub(super) type Accounts<T: Config> =
		StorageMap<_, Blake2_128Concat, T::AccountId, AccountStatus<BalanceOf<T>>, ValueQuery>;

	// The account that will be used to payout participants of the STC purchase process.
	#[pallet::storage]
	...
	// The block where all locked stcs will unlock.
	#[pallet::storage]
	...
	impl<T: Config> Pallet<T> {
			...
			Accounts::<T>::try_mutate(
				...
				|status: &mut AccountStatus<BalanceOf<T>>| -> DispatchResult {
					...
					if !status.locked_balance.is_zero() {
						...
						// We allow some configurable portion of the purchased locked STCs to be
		...
		/// Set the account that will be used to payout users in the STC purchase process.
		///
		...
		/// Set the statement that must be signed for a user to participate on the STC sale.
		///
		/// Origin must match the `ConfigurationOrigin`
		#[pallet::call_index(5)]
		#[pallet::weight(T::DbWeight::get().writes(1))]
		...
		/// Set the block where locked STCs will become unlocked.
		///
		/// Origin must match the `ConfigurationOrigin`
		#[pallet::call_index(6)]
		#[pallet::weight(T::DbWeight::get().writes(1))]
...
impl<T: Config> Pallet<T> {
	fn verify_signature(who: &T::AccountId, signature: &[u8]) -> Result<(), DispatchError> {
		...
		// In SaitaChain, the AccountId is always the same as the 32 byte public key.
