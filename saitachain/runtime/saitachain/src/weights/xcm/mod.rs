...
/// Types of asset supported by the SaitaChain runtime.
pub enum AssetTypes {
...
// Saitama only knows about one asset, the balances pallet.
...
impl WeighMultiAssets for MultiAssetFilter {
	fn weigh_multi_assets(&self, balances_weight: Weight) -> Weight {
		match self {
			...
			// We don't support any NFTs on SaitaChain, so these two variants will always match
...
pub struct SAITACHAINXcmWeight<RuntimeCall>(core::marker::PhantomData<RuntimeCall>);
impl<RuntimeCall> XcmWeightInfo<RuntimeCall> for SAITACHAINXcmWeight<RuntimeCall> {
	...
	fn reserve_asset_deposited(assets: &MultiAssets) -> Weight {
		// Saitama doesn't support ReserveAssetDeposited, so this benchmark has a default weight
	...
	fn exchange_asset(_give: &MultiAssetFilter, _receive: &MultiAssets, _maximal: &bool) -> Weight {
		// Saitama does not currently support exchange asset operations
		Weight::MAX
	}
	...
	fn universal_origin(_: &Junction) -> Weight {
		// Saitama does not currently support universal origin operations
		Weight::MAX
	}
	fn export_message(_: &NetworkId, _: &Junctions, _: &Xcm<()>) -> Weight {
		// Saitama relay should not support export message operations
		Weight::MAX
	}
	fn lock_asset(_: &MultiAsset, _: &MultiLocation) -> Weight {
		// Saitama does not currently support asset locking operations
		Weight::MAX
	}
