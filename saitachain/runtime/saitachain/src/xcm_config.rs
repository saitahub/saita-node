...
parameter_types! {
	...
	pub const STCForStatemint: (MultiAssetFilter, MultiLocation) = (Stc::get(), StatemintLocation::get());
	pub const CollectivesLocation: MultiLocation = Parachain(COLLECTIVES_ID).into_location();
	pub const StcForCollectives: (MultiAssetFilter, MultiLocation) = (Stc::get(), CollectivesLocation::get());
	pub const MaxAssetsIntoHolding: u32 = 64;
}

/// Saitama Relay recognizes/respects the Statemint chain as a teleporter.
pub type TrustedTeleporters =
	(xcm_builder::Case<STCForStatemint>, xcm_builder::Case<StcForCollectives>);
