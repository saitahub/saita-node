...
async fn purge_chain_rocksdb_works() {
	...
	let mut cmd = Command::new(cargo_bin("saitachain"))
	...
	assert!(tmpdir.path().join("chains/saitachain_dev").exists());
	assert!(tmpdir.path().join("chains/saitachain_dev/db/full").exists());
	assert!(tmpdir.path().join("chains/saitachain_dev/db/full/parachains").exists());

	// Purge chain
	let status = Command::new(cargo_bin("saitachain"))
...
async fn purge_chain_paritydb_works() {
	...
	assert!(common::wait_for(&mut cmd, 30).map(|x| x.success()).unwrap_or_default());
	assert!(tmpdir.path().join("chains/saitachain_dev").exists());
	assert!(tmpdir.path().join("chains/saitachain_dev/paritydb/full").exists());
	assert!(tmpdir.path().join("chains/saitachain_dev/paritydb/parachains").exists());
