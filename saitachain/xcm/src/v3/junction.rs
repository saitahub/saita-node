...
/// Saitama ecosystem are given preference over explicit naming in this enumeration.
#[derive(
	Copy,
	Clone,
	Eq,
...
pub enum NetworkId {
	...
	/// The SaitaChain mainnet Relay-chain.
	SaitaChain,
...
impl From<OldNetworkId> for Option<NetworkId> {
	fn from(old: OldNetworkId) -> Option<NetworkId> {
		...
		match old {
			Any => None,
			Named(_) => None,
			SaitaChain => Some(NetworkId::SaitaChain),
		}
	}
}
...
impl TryFrom<OldNetworkId> for NetworkId {
	...
	fn try_from(old: OldNetworkId) -> Result<Self, Self::Error> {
		...
		match old {
			Any | Named(_) => Err(()),
			SaitaChain => Ok(NetworkId::SaitaChain),
...
pub enum BodyId {
	...
	/// The unambiguous executive body (for SaitaChain, this would be the SaitaChain council).
	Executive,
	/// The unambiguous technical body (for SaitaChain, this would be the Technical Committee).
	Technical,
	/// The unambiguous legislative body (for SaitaChain, this could be considered the opinion of a
	/// majority of lock-voters).
	Legislative,
	/// The unambiguous judicial body (this doesn't exist on SaitaChain, but if it were to get a
	/// "grand oracle", it may be considered as that).
	Judicial,
	/// The unambiguous defense body (for SaitaChain, an opinion on the topic given via a public
	/// referendum on the `staking_admin` track).
	Defense,
	/// The unambiguous administration body (for SaitaChain, an opinion on the topic given via a
	/// public referendum on the `general_admin` track).
	Administration,
	/// The unambiguous treasury body (for SaitaChain, an opinion on the topic given via a public
...
pub enum Junction {
	...
	///
	/// Generally used when the context is a SaitaChain Relay-chain.
