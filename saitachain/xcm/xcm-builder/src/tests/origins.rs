...
fn export_message_should_work() {
	...
	// Local parachain #1 issues a transfer asset on SaitaChain Relay-chain, transfering 100 Planck to
	// Saitama parachain #2.
	...
	assert_eq!(
		exported_xcm(),
		vec![(SaitaChain, 403611790, uni_src, Here, expected_message, expected_hash)]
	);
}
