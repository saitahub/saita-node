...
mod tests {
	...
	fn ensure_is_remote_works() {
		...
		let x = ensure_is_remote(SaitaChain, (Parent, SaitaChain, Parachain(1000)));
		assert_eq!(x, Ok((SaitaChain, Parachain(1000).into())));

		// Our own parachain is local.
		let x = ensure_is_remote(SaitaChain, Parachain(1000));
		assert_eq!(x, Err(Parachain(1000).into()));

		// Saitama's parachain is not remote if we are SaitaChain.
		let x = ensure_is_remote(SaitaChain, (Parent, SaitaChain, Parachain(1000)));
		assert_eq!(x, Err((Parent, SaitaChain, Parachain(1000)).into()));

		// If we don't have a consensus ancestor, then we cannot determine remoteness.
		let x = ensure_is_remote((), (Parent, SaitaChain, Parachain(1000)));
		assert_eq!(x, Err((Parent, SaitaChain, Parachain(1000)).into()));
	}
}
