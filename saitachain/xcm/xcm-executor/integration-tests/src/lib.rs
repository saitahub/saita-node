...
use saitama_test_client::{
	BlockBuilderExt, ClientBlockImportExt, DefaultTestClientBuilderExt, InitSaitaChainBlockBuilder,
	TestClientBuilder, TestClientBuilderExt,
};
use saitachain_test_runtime::{pallet_test_notifier, xcm_config::XcmConfig};
use saitama_test_service::construct_extrinsic;
...
fn basic_buy_fees_message_executes() {
	...
	let mut block_builder = client.init_saitachain_block_builder();

	let execute = construct_extrinsic(
		&client,
		saitachain_test_runtime::RuntimeCall::Xcm(pallet_xcm::Call::execute {
	...
	block_builder.push_saitachain_extrinsic(execute).expect("pushes extrinsic");
	...
	client.state_at(block_hash).expect("state should exist").inspect_state(|| {
		assert!(saitachain_test_runtime::System::events().iter().any(|r| matches!(
			r.event,
			saitachain_test_runtime::RuntimeEvent::Xcm(pallet_xcm::Event::Attempted {
...
fn transact_recursion_limit_works() {
	...
	let mut call = saitachain_test_runtime::RuntimeCall::Xcm(pallet_xcm::Call::execute {
		message: Box::new(VersionedXcm::from(msg)),
		max_weight,
	});
	...
	for _ in 0..11 {
		...
		let max_weight = <XcmConfig as xcm_executor::Config>::Weigher::weight(&mut msg).unwrap();
		call = saitachain_test_runtime::RuntimeCall::Xcm(pallet_xcm::Call::execute {
			message: Box::new(VersionedXcm::from(msg)),
			max_weight,
		});
	}
...
fn query_response_fires() {
	...
	use saitachain_test_runtime::RuntimeEvent::TestNotifier;
	...
	let execute = construct_extrinsic(
		&client,
		saitachain_test_runtime::RuntimeCall::TestNotifier(
	...
	client.state_at(block_hash).expect("state should exist").inspect_state(|| {
		for r in saitachain_test_runtime::System::events().iter() {
			match r.event {
	...
	client.state_at(block_hash).expect("state should exist").inspect_state(|| {
		assert!(saitachain_test_runtime::System::events().iter().any(|r| matches!(
			r.event,
			saitachain_test_runtime::RuntimeEvent::Xcm(pallet_xcm::Event::ResponseReady {
				query_id: q,
				response: Response::ExecutionResult(None),
			}) if q == query_id,
		)));
		assert_eq!(
			saitachain_test_runtime::Xcm::query(query_id),
